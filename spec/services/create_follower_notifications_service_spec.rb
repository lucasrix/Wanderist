...
describe CreateFollowerNotificationsService do
  ...
  describe '#set_params' do
    ...
    before do
      allow(subject).to receive(:set_following_event_params)
      allow(subject).to receive(:set_user_event_params)
    end

    after do
      subject.send(:set_params, resource, event)
    end

    context 'resource is a Following' do
      it 'calls' do
        allow(resource).to receive(:is_a?).and_return(true)
        expect(subject).to receive(:set_following_event_params)
      end
    end

    context 'resource isnt a Following' do
      it 'calls' do
        allow(resource).to receive(:is_a?).and_return(false)
        expect(subject).to receive(:set_user_event_params)
      end
    end
  end

  describe '#set_following_event_params' do
    let(:another_user) { create(:user) }
    let(:resource) { create(:following, followable: story, user: another_user) }

    before do
      subject.send(:set_following_event_params, resource, :new_following)
    end
  ...
  describe '#set_user_event_params' do
    let(:another_user) { create(:user) }
    let(:resource) { create(:story, user: another_user) }

    before do
      subject.send(:set_user_event_params, resource, :new_story)
    end
